...
public class MemberChooser<T extends ClassMember> extends DialogWrapper implements TypeSafeDataProvider {
        ...
        FactoryMap<MemberChooserObject, ParentNode> map = new FactoryMap<MemberChooserObject, ParentNode>() {
            ...
            protected ParentNode create(MemberChooserObject key) {
                ...
                if (isContainerNode(key)) {
                    ContainerNode containerNode = new ContainerNode(parentNode, key, count);
                    node = containerNode;
    ...
    public void selectElements(ClassMember[] elements) {
        ...
        TreePath[] paths = selectionPaths.toArray(new TreePath[selectionPaths.size()]);
    ...
    protected void installSpeedSearch() {
        TreeSpeedSearch treeSpeedSearch = new TreeSpeedSearch(myTree, new Convertor<TreePath, String>() {
            @Override
            @Nullable
    ...
    protected boolean areElementsSelected() {
